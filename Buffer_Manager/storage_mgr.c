...
RC readPreviousBlock(SM_FileHandle *fHandle, SM_PageHandle memPage) {
	...
	int currentCursorPosition = 0;//fHandle->mgmtInfo;
	int currentBlock = (currentCursorPosition / PAGE_SIZE);
	int previousBlock;
	if (currentBlock == 0)
		previousBlock = 0;
	else
		//Eg.,CursorPosition(Block Number) = 300(0),4096(0),5000(0)8012(1)
...
RC readCurrentBlock(SM_FileHandle *fHandle, SM_PageHandle memPage) {
	...
	int currentBlock;
	int currentCursorPosition = 0; //fHandle->mgmtInfo;//Eg.,CursorPosition(Block Number) = 300(0),4096(1),5000(1)8012(2)
...
RC readNextBlock(SM_FileHandle *fHandle, SM_PageHandle memPage) {
	...
	int nextBlock;
	int currentCursorPosition = 0;//fHandle->mgmtInfo; //Eg.,CursorPosition(Block Number) = 300(1),4096(2),5000(2)8012(3)
...
RC ensureCapacity(int numberOfPages, SM_FileHandle *fHandle) {
	...
	if (ADD_BLOCKS <= 0) {
		...
		printf("Total number of blocks are %d \n",
				getTotalNumberOfBlocks(fpAEB) + 1);
...
RC readBlocks(FILE *fpRB, SM_FileHandle *fHandle, SM_PageHandle memPage) {
	...
	if (currentBlock > TOTAL_BLOCKS || currentBlock < 0) {
		...
		printf("IF--->Bytes Read are :- %d and memPage is %s \n", bytesRead,
				memPage);
